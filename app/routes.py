...
from .models import User, OTP, Admin, db, Prediction, Book, Rating, Users, Wishlist, Feedback
...
def signup():
    if request.method == 'POST':
        ...
        city = request.form.get('city', "Unknown")  # ✅ Ensure `city` exists
        state = request.form.get('state', "Unknown")  # ✅ Ensure `state` exists
        country = request.form.get('country', "Unknown")  # ✅ Ensure `country` exists
        security_question = request.form.get('security_question')
        security_answer = encrypt_security_answer(request.form.get('security_answer'))
        ...
        # ✅ Ensure session contains all values
        session['signup_data'] = {
        ...
        print(f"DEBUG: Signup Data Stored in Session = {session['signup_data']}")  # Debugging

        # ✅ Generate OTP
        otp = random.randint(100000, 999999)
        session['signup_data']['otp'] = otp
        print(f"DEBUG: Generated OTP for signup = {otp}")  # Debugging
...
def verify_signup_otp():
    ...
    # ✅ Debugging: Print all keys in `signup_data`
    print(f"DEBUG: Signup Data Keys: {list(signup_data.keys())}")

    if 'city' not in signup_data:
        flash("City information is missing. Please re-enter your details.", "danger")
        return redirect(url_for("main.signup"))
    ...
    if request.method == 'POST':
        ...
        new_user = User(
            ...
            city=signup_data.get('city', "Unknown"),
            state=signup_data.get('state', "Unknown"),
            country=signup_data.get('country', "Unknown")
        )
...
def dashboard():
    ...
    # Fetch user's feedback on books
    feedbacks = (
        db.session.query(Book.book_title, Book.book_author, Book.genre, Feedback.feedback, Feedback.submitted_on)
        .join(Feedback, Book.ISBN == Feedback.ISBN)
        .filter(Feedback.user_id == user_id)
        .order_by(Feedback.submitted_on.desc())
        .all()
    )
    feedbacks_df = pd.DataFrame(feedbacks, columns=["Title", "Author", "Genre", "Feedback", "Submitted On"])
...
@main.route("/add_to_wishlist/<isbn>", methods=["POST"])
def add_to_wishlist(isbn):
    ...
    existing_wishlist = Wishlist.query.filter_by(user_id=user_id, ISBN=isbn).first()
    if existing_wishlist:
        flash("This book is already in your wishlist.", "info")
        return redirect(url_for("main.book_details", isbn=isbn))

    # Add book to wishlist
    new_wishlist_entry = Wishlist(user_id=user_id, ISBN=isbn)
...
@main.route('/submit_feedback/<isbn>', methods=['POST'])
def submit_feedback(isbn):
    if 'user_id' not in session:
    ...
    user = User.query.filter_by(id=user_id).first()  # ✅ Use `User` model instead of `Users`

    if not book:
        flash("❌ Book not found.", "danger")
        return redirect(url_for('main.browse_books'))

    feedback_text = request.form.get("feedback")

    # Save feedback to database
    feedback_entry = Feedback(user_id=user_id, ISBN=isbn, feedback=feedback_text)
    db.session.add(feedback_entry)
    db.session.commit()

    # Ensure user has an email before sending confirmation
    if user and user.email:
        send_email(
            to_email=user.email,
            subject="📚 Book Feedback Submitted!",
            body=f"""
            Hello {user.name},

            Thank you for submitting your feedback on "{book.book_title}".

            Your feedback: "{feedback_text}"

            Best Regards,
            Your Book Recommendation System Team
            """
        )

    flash("✅ Feedback submitted! A confirmation email has been sent.", "success")
    return redirect(url_for('main.book_details', isbn=isbn))

@main.route('/admin_feedbacks', methods=['GET', 'POST'])
def admin_feedbacks():
    if not session.get('admin'):  # ✅ Ensure only admins can access
        return redirect(url_for('main.admin_login'))

    # ✅ Explicitly join User and Book to prevent DetachedInstanceError
    feedbacks = db.session.query(Feedback, User.name, Book.book_title) \
        .join(User, Feedback.user_id == User.id) \
        .join(Book, Feedback.ISBN == Book.ISBN) \
        .order_by(Feedback.submitted_on.desc()).all()
...
def wishlist():
    ...
    wishlist_books = (
        db.session.query(Book.ISBN, Book.book_title, Book.book_author, Book.genre, Book.publisher, Book.year_of_publication)
        .join(Wishlist, Book.ISBN == Wishlist.ISBN)
        .filter(Wishlist.user_id == user_id)
        .all()
    )

    wishlist_df = pd.DataFrame(wishlist_books, columns=["ISBN", "Title", "Author", "Genre", "Publisher", "Year"])

    # Handle book removal from wishlist
    if request.method == "POST":
        isbn = request.form.get("isbn")
        wishlist_item = Wishlist.query.filter_by(user_id=user_id, ISBN=isbn).first()

        if wishlist_item:
            db.session.delete(wishlist_item)
            db.session.commit()
            flash("Book removed from wishlist.", "success")
            return redirect(url_for("main.wishlist"))  # Refresh page after removing item

    return render_template("wishlist.html", wishlist=wishlist_df)

@main.route("/rate_book/<isbn>", methods=["POST"])
def rate_book(isbn):
    if "user_id" not in session:
    ...
    existing_rating = Rating.query.filter_by(user_id=user_id, ISBN=isbn).first()
    
    if existing_rating:
        existing_rating.book_rating = new_rating  # Update existing rating
...
def security_question():
    ...
    if request.method == 'POST':
        entered_answer = request.form.get('security_answer', "").strip()

        # ✅ Validate using hashed security answer check
        if check_security_answer(entered_answer, user.security_answer):  
            otp = random.randint(100000, 999999)
...
def reset_password():
    ...
    if request.method == 'POST':
        ...
        # ✅ Validate hashed security answer
        if not check_security_answer(entered_answer, user.security_answer):  
            flash("Incorrect security answer!", "danger")
            return redirect(url_for("main.reset_password"))
    ...
    return render_template('reset_password.html', security_question=user.security_question)
